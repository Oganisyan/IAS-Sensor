...
MS5611::MS5611(TwoWire &wire) :
		wire_(wire), address_(0), callOrderTime_(millis() + 500), callOrderCode_(
				0x00) {
...
uint32_t MS5611::getValue() {
	...
	if (wire_.available() >= 3) {
		ret = ((uint32_t) wire_.read()) << 16 | ((uint32_t) wire_.read()) << 8
				| ((uint32_t) wire_.read());
	} else {
		ret = INVALID_UNIT32;
	}
...
void MS5611::loop() {
	if (address_ == 0 || callOrderTime_ > millis()) {
		return;
	}
...
double MS5611::getTemperature() {
	return 0.01 * temperature_;
}

double MS5611::getPressure() {
	return 0.01 * pressure_;
}
