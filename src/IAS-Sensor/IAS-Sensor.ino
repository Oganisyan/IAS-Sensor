...
#define PIN_BTN GPIO_NUM_34  // POW BUTTON
#define PIN_BT2 GPIO_NUM_19  // BUTTON_2 NOT USED
#define PIN_ADC GPIO_NUM_36  // BATARY VOLTAGE /2

#define LOW_BATARY_VOLTAGE	3.5
...
const char* getLXWP0(double p, double ias) {
	...
	snprintf(buffer, 128, LPWX0_FOTMATER, d2s(ias).c_str(), d2s(alt).c_str(),
			d2s(vario).c_str(), 0x00);
	snprintf(buffer, 128, LPWX0_FOTMATER, d2s(ias).c_str(), d2s(alt).c_str(),
			d2s(vario).c_str(), calcCheckSum(buffer));
...
const char* getLXWP1() {
	...
	snprintf(buffer, 128, LPWX1_FOTMATER, DEVICE_NAME, serialNumber, SV_VERSION,
			HV_VERSION, 0x00);
	snprintf(buffer, 128, LPWX1_FOTMATER, DEVICE_NAME, serialNumber, SV_VERSION,
			HV_VERSION, calcCheckSum(buffer));
	DBG(Serial.print(buffer));
	return buffer;
}
...
void loop() {
    double voltage = powerManager->get();
    if(voltage < LOW_BATARY_VOLTAGE) {
	...
	if (pServer->isConnected()) {
        ...
        pServer->sendBatteryLevel(voltage);
	} else {
		DBG(Serial.println("startAdvertising");)
